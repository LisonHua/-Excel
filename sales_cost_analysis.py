...
EXCLUDED_SUPPLIERS = {"天音通信有限公司", "TELLING TELECOM（HK）LIMITED"}
...
def _apply_column_aliases(df: pd.DataFrame, alias_map: dict) -> pd.DataFrame:
    """Return a copy of *df* with missing columns filled using aliases."""
...
def _normalise_material_values(series: pd.Series) -> pd.Series:
...
def _load_dataframe(data: Union[str, Path, pd.DataFrame]) -> pd.DataFrame:
    ...
    try:
        ...
        dependency = dependency_hints.get(suffix, "openpyxl")
        raise ImportError(
            (
                "读取 Excel 文件需要安装 '{dep}' 库，请先运行 pip install {dep} 再重试，"
                "或将文件另存为 CSV 格式。"
...
def augment_sales_with_costs(
    ...
    sales = _apply_column_aliases(sales, {"物料编码": ["物料"]})

    required_sales_columns = {"创建日期", "订单类型", "物料编码", "售达方"}
    missing_sales = required_sales_columns - set(sales.columns)
    if missing_sales:
        raise KeyError(
            "销售订单明细表缺少列: {}".format(", ".join(sorted(missing_sales)))
        )

    if {"售达方", "区域"} - set(regions.columns):
        missing = {"售达方", "区域"} - set(regions.columns)
        raise KeyError("客户销售区域表缺少列: {}".format(", ".join(sorted(missing))))
    ...
    purchases = _apply_column_aliases(
        purchases,
        {"物料编码": ["物料"], "过账日期": ["过帐日期"], "币种": ["币别"]},
    )
    purchases = purchases.copy()
    purchases["物料编码"] = _normalise_material_values(purchases["物料编码"])
    purchases["过账日期"] = _ensure_datetime(purchases, "过账日期")
    if "供应商名称" in purchases.columns:
        supplier_series = purchases["供应商名称"].astype("string").str.strip()
        purchases = purchases[~supplier_series.isin(EXCLUDED_SUPPLIERS)]
...
def process_and_export(
    ...
    if output_path is not None:
        ...
        if suffix == ".csv":
            sales_augmented.to_csv(output_path, index=False)
        else:
            try:
                sales_augmented.to_excel(output_path, index=False)
            except ImportError as exc:
                raise ImportError(
                    "保存为 Excel 文件需要安装 'openpyxl' 库，请先运行 pip install openpyxl "
                    "再重试，或选择保存为 CSV 格式。"
                ) from exc
...
def _select_paths_with_gui() -> Tuple[str, str, str, str, Optional[str]]:
    """Use a simple GUI to collect the required input/output file paths."""
    ...
    try:
        filetypes = [
            ("Excel 或 CSV 文件", "*.xlsx *.xls *.csv"),
            ("所有文件", "*.*"),
        ]

        sales = filedialog.askopenfilename(
            title="请选择销售订单明细表文件",
            filetypes=filetypes,
        )
        if not sales:
            raise RuntimeError("未选择销售订单明细表文件")

        purchases = filedialog.askopenfilename(
            title="请选择采购入库明细表文件",
            filetypes=filetypes,
        )
        if not purchases:
            raise RuntimeError("未选择采购入库明细表文件")
        ...
        if not fx:
            raise RuntimeError("未选择汇率表文件")

        regions = filedialog.askopenfilename(
            title="请选择客户销售区域表文件",
            filetypes=filetypes,
        )
        if not regions:
            raise RuntimeError("未选择客户销售区域表文件")
        ...
        return sales, purchases, fx, regions, output
    finally:
        root.destroy()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        try:
            (
                ...
                region_path,
                output_path,
            ) = _select_paths_with_gui()
            used_gui = True
        except Exception as exc:  # noqa: BLE001 - want to show any GUI issues
            parser.print_help(sys.stderr)
            parser.exit(1, f"无法通过图形界面获取文件路径: {exc}\n")
    else:
    ...
    try:
        result_df, failures = process_and_export(
            sales_path, purchase_path, fx_path, region_path, output_path
        )
    except ImportError as exc:
        message = str(exc)
        if used_gui:
